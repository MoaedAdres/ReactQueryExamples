...
const OptimisticUpdate = () => {
    ...
    const { isLoading: customLoadingName, data, isFetching, isError, error, refetch } = fetchHeros(onSuccess, onError)
    ...
    return (
        <div>
            ...
            {data?.data?.map((hero, index) => <h1>{hero.name}id is {hero.id}</h1>)}
        </div>
    )
}
